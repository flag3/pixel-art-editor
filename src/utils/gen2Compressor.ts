...
function tryCompressSinglePass(
  data: Uint8Array,
...
function findBestCopy(
  ...
  const simpleResult = scanForwards(
    data.slice(position),
    length - position,
    data,
    position,
  ...
  if (simpleResult.count > 0) {
    simple = {
      ...
      value: simpleResult.offset,
    };
  }

  // Scan forwards for bit-flipped copy
  const flippedResult = scanForwards(
    data.slice(position),
    length - position,
    bitflipped,
    position,
  ...
  if (flippedResult.count > 0) {
    flipped = {
      command: 5,
      count: flippedResult.count,
      value: flippedResult.offset,
    };
  }
  ...
  if (backwardsResult.count > 0) {
    backwards = {
      command: 6,
      count: backwardsResult.count,
      value: backwardsResult.offset,
    };
  }
...
function findBestRepetition(
  ...
  if (position + 1 >= length) {
    return data[position]
      ? { command: 7, count: 0, value: 0 }
      : { command: 3, count: 1, value: 0 };
  }
  ...
  while (
    repcount < limit &&
    data[position + repcount] === value[repcount & 1]
  ) {
    repcount++;
  }
  ...
  while (next < commands.length) {
    ...
    if (commands[next].command === commands[current].command) {
      switch (commands[current].command) {
        case 0: // literal
          if (
            commands[current].value + commands[current].count ===
            commands[next].value
          ) {
            commands[current].count += commands[next].count;
